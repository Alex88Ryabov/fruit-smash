...
// нужны three и playwright: npm install в папке tools и распакованный kenney_food-kit.zip
// с https://kenney.nl/assets/food-kit — путь к Models/GLB format
...
const outDir = process.argv[3] || path.join(TOOLS, '..', 'art', 'sprites');
if (!modelsDir) {
...
// страница и three.js отдаются из папки tools, модели — из переданной под /models/
const server = http.createServer((req, res) => {
  const url = decodeURIComponent(req.url.split('?')[0]);
  const file = url.startsWith('/models/') ? path.join(modelsDir, url.slice(8)) : path.join(TOOLS, url);
...
(async () => {
  ...
  await page.goto('http://localhost:' + port + '/sprites/page.html');
